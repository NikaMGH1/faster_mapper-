...
public:
	...
	void write_line(const string& path, const string& key, const string& line) {
		string dir_string = extract_dir_name(key);
		//////////////////////////////////////////////////////////////
		// check if we need to create a dir. Only 1 thread should be in here at a time
        struct stat s;
int err = stat((path + dir_string).c_str(), &s);
if(-1 == err) {
    if(ENOENT == errno) {
       mkdir((path + dir_string).c_str(), S_IRWXU);/* does not exist */
    } else {
        perror("stat");
        exit(1);
    }
		...
		// just touch the file
		string file_total_name = path + dir_string + key + ".sam";
		//ofstream output_file(file_total_name, fstream::out);
        
		//output_file.close();
//////////////////////////////////////////////////////////////////////////		
		//string file_total_name = path + dir_string + key + ".sam";

		/* check whether or not we need to open anything*/
		ofstream output_file(file_total_name, fstream::app);
		
		output_file << line << endl;
		/*check what we have to close*/
		output_file.close();
	};

	/* void check_dir(const string& path, const string& key) {
		string dir_string = extract_dir_name(key);

		// check if we need to create a dir. Only 1 thread should be in here at a time
		//DIR* dir_ptr = opendir((path + dir_string).c_str());
		//changed
		
		//if (dir_ptr == NULL) {
		//if((statbuf.st_mode & S_IFDIR)  == S_IFDIR) 
		  //if( !S_ISDIR((path + dir_string).c_str())) {
...
if(-1 == err) {
    if(ENOENT == errno) {
       mkdir((path + dir_string).c_str(), S_IRWXU); //does not exist 
		...
		////////////////////////////////////////////////////////////////////////////////////
		/*  if( ((path + dir_string)->st_mode & S_IFMT) != S_IFDIR) {
			mkdir((path + dir_string).c_str(), S_IRWXU);
		}
		else
			//closedir(dir_ptr);
			*/
	...
	//	string file_total_name = path + dir_string + key + ".sam";
		...
		/* while (!output_file.is_open() ) {
			cerr << "File " << file_total_name << " trying again" << endl;
			output_file.open(file_total_name, fstream::out);
		} */
	/*	output_file.close();
...
void process_lines(string *lines, int num_lines, map<string, Barcode_st>& barcode_map, omp_lock_t *read_locks, int num_locks, string output_dir = "./") {
	...
	for (i = 0; i < num_lines; i++) {
		...
		/*
		// finding BC
		int BC_start = find_pattern_start(line, "BC:Z:");
		
		if (BC_start != -1) {
			int BC_length = find_length(line, BC_start);
			BC = line.substr(BC_start, BC_length);
		}
		*/
		...
		if (BX_start != -1) { // && BC_start != -1) {
			...
			if (key_exist) {
				...
				barcode_ptr->write_line(output_dir, barcode_key, line);
...
//t1=rdtsc();
	...
	barcode_map["no_barcode"].write_line(output_dir, "no_barcode", "");
